...
export const Produtos = () => {
  ...
  const {
    ...
    handleDragLeave,
    handleDrop,
  }: any = useArrastaSolta(produtos, setProdutos);
  ...
  return (
    <div className="flex flex-col w-full h-full gap-5">
      ...
      {/* Mensagens de Status */}
      {carregando && (
        <p className="text-muted-foreground text-center animate-pulse">Buscando dados...</p>
      )}
          ...
          <table className="w-full border-collapse text-sm mt-5 table-auto text-left">
            <thead className="bg-card text-muted-foreground font-medium uppercase border-y border-border">
              <tr>
                ...
                <th className="p-3 w-full min-w-50">Descrição</th>
                ...
                <th className="p-3 whitespace-nowrap text-right min-w-45">Ações</th>
              </tr>
            </thead>
            <tbody>
              {produtosFiltrados.map((produto: any, index: number) => (
                <tr
