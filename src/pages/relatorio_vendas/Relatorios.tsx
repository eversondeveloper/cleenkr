...
export const Relatorios = () => {
  ...
  const {
    ...
    vendasFiltradas,
    retiradasFiltradas,
  }: any = useFiltros(vendas, retiradasFiltradasDoHook, filtrarRetiradasLocalmente);
  ...
  const {
    ...
    totalLiquido,
    quantidadeVendas,
  }: any = useCalculos(vendasFiltradas, retiradasFiltradas);

  const { gerarPDF, gerandoPDF }: any = useGeracaoPDF();
  ...
  const [mostrarModalObs, setMostrarModalObs] = useState<boolean>(false);
  ...
  const [modalEdicaoVenda, setModalEdicaoVenda] = useState<boolean>(false);
  ...
  // --- HANDLERS DE VENDAS ---
  const handleDeletarVenda = async (id: any) => {
  ...
  const handleAtualizarVenda = async (idVenda: any, novosPagamentos: any) => {
  ...
  const handleDeletarRetirada = async (id: any) => {
  ...
  const handleAbrirObsParaData = (data: string, texto = '') => {
    setDataEdicao(data);
  ...
  const handleBotaoPrincipalObservacao = () => {
    ...
    const obsExistente = observacoes.find(
      (obs: any) => obs.data_observacao.split('T')[0] === dataAlvo
    );
  ...
  if (carregando || carregandoEmpresa) {
    return (
      <div className="flex w-[98%] h-full my-5 mx-auto items-center justify-center bg-card rounded-xl shadow-2xl border border-border">
        <h1 className="text-3xl font-light text-muted-foreground animate-pulse">
    ...
    <div className="flex flex-col w-full h-full gap-5">
      <div className="w-full text-center mb-8 pb-4 border-b border-border">
        ...
        <p className="text-base text-muted-foreground m-0">
          Acompanhamento de vendas, retiradas e fluxo de caixa.
        </p>
      </div>
      ...
      {observacoes.length > 0 && (
        <div className="w-full mt-8 mb-5">
          ...
          {observacoes.map((obs: any) => {
            ...
            return (
              <div
                key={obs.data_observacao}
                className="bg-card p-5 mb-5 rounded-lg w-full border border-border shadow-sm"
              >
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-muted-foreground text-base m-0 font-bold">
                    Data: {dataFormatada}
                  </h3>
                  <button
                    ...
                    onClick={() =>
                      handleAbrirObsParaData(obs.data_observacao.split('T')[0], obs.texto)
                    }
                  >
                    Editar Nota
                  </button>
