...
export const CadastroAtendentes = () => {
  ...
  const {
    ...
    atendentesFiltrados,
    handleLimparFiltros,
  }: any = useFiltrosAtendentes(atendentesBase || []);

  const [mostrarModalAtendente, setMostrarModalAtendente] = useState<boolean>(false);
  const [mostrarModalSessao, setMostrarModalSessao] = useState<boolean>(false);
  const [atendenteEditando, setAtendenteEditando] = useState<any>(null);
  const [atendenteSelecionadoParaSessao, setAtendenteSelecionadoParaSessao] = useState<any>(null);
  ...
  const handleSalvarAtendente = async (dadosAtendente: any) => {
  ...
  const handleDeletarAtendente = async (id: any) => {
  ...
  const handleAbrirSessao = async (atendenteId: any, valorInicial = 0) => {
    const idEmpresaFinal = empresa?.id_empresa || empresa?.id;
  ...
  const handleExcluirEmpresaDefinitivo = async (id: any) => {
    if (
      window.confirm(
        '⚠️ PERIGO: Isso apagará TODOS os dados da empresa (Vendas, Produtos, Operadores). Deseja continuar?'
      )
    ) {
    ...
    <div className="flex flex-col w-full h-full gap-5">
      ...
      <div className="p-5 bg-card rounded-lg border border-border">
        <SecaoAcoesAtendentes
          onNovoAtendente={() => {
            setAtendenteEditando(null);
            setMostrarModalAtendente(true);
          }}
          onAbrirSessao={() => {
            setAtendenteSelecionadoParaSessao(null);
            setMostrarModalSessao(true);
          }}
      ...
      <div className="flex flex-col w-full">
        <div className="flex justify-between items-center mb-5">
          <h3 className="text-primary text-lg font-normal m-0">📋 Operadores Cadastrados</h3>
        ...
        {atendentesFiltrados?.length > 0 ? (
          <TabelaAtendentes
            ...
            carregando={carregandoAtendentes}
            sessaoAtual={sessaoAtual}
            onEditarAtendente={(a: any) => {
              setAtendenteEditando(a);
              setMostrarModalAtendente(true);
            }}
            onDeletarAtendente={handleDeletarAtendente}
            onAbrirSessao={(id: any) => {
              setAtendenteSelecionadoParaSessao(id);
              setMostrarModalSessao(true);
            }}
          />
        ) : (
          <div className="p-14 text-center bg-card rounded-xl text-muted-foreground border border-dashed border-border">
            <p className="text-base m-0">
              Nenhum operador encontrado para os critérios selecionados.
            </p>
          </div>
        )}
      </div>
