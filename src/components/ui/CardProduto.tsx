// src/components/ui/CardProduto.tsx
...
const CardProduto: FC<CardProdutoProps> = ({
  ...
  const corCategoria = $corFundo || COR_PADRAO_DEFAULT;
  const produtoNoCarrinho = $produtosSelecionados.find(
    (p) => p.id_produto === $id
  );
  const estaSelecionado = !!produtoNoCarrinho;
  const quantidadeNoCarrinho = produtoNoCarrinho?.quantidade ?? 0;
...
const comparadorDeProps = (
  prev: Readonly<CardProdutoProps>,
  next: Readonly<CardProdutoProps>
): boolean => {
  ...
  return (
    ...
    prev.$corFundo === next.$corFundo // ← Agora o memo também verifica se a cor do banco mudou
  );
};

export default memo(CardProduto, comparadorDeProps);