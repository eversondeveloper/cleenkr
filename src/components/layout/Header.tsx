...
export function Header({
  ...
  const handleLogout = async () => {
    if (window.confirm('Deseja realmente sair do sistema?')) {
      ...
      navigate('/scleenkr/login', { replace: true });
    }
  };

  return (
    <header className="w-full h-[6%] min-h-12.5 flex items-center justify-center bg-card border-b border-border z-50">
      <div className="w-[98%] flex items-center justify-between">
        {/* LOGO */}
        <div className="h-[80%] w-32.5 flex items-center">
            ...
            {sessaoAtual && (
              <div className="flex items-center gap-1.5 text-[#888] text-[13px]">
                ...
                <strong className="text-success font-semibold capitalize">
                  {(sessaoAtual as any).nome_atendente ?? 'Operador'}
                </strong>
              </div>
            )}
          </div>
        ...
        <nav className="relative flex items-center h-full gap-4">
          ...
          <ul
            ...
            p-2.5 min-w-50 flex flex-col gap-1.25 z-50 transition-all duration-200 origin-top-right
          ...
          >
            {[
              ...
              <li key={index} className="w-full">
                <Link
                  to={link.path}
                  className="block w-full text-left bg-secondary text-muted-foreground border border-border rounded-xl 
                  px-3.75 py-2.5 text-[14px] font-normal leading-normal transition-all duration-200 cursor-pointer 
